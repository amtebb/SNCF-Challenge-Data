...
def enhanced_encoding(X_train_full, X_test_full, y_train_full):
    """
    Fonction avancée pour l'encodage et la création de caractéristiques.
    
    Args:
        X_train_full: DataFrame d'entraînement avec features
        X_test_full: DataFrame de test avec features
        y_train_full: DataFrame d'entraînement avec la cible (p0q0)
    ...
    # Conversion des dates et extraction de caractéristiques temporelles
    ...
    def get_tension_score(row):
        """Calcule un score de tension selon l'heure et le jour de la semaine"""
        day = row['day_of_week']
        hour = row['hour']
        
        # Heures de pointe avec leurs poids respectifs
        peak_hours = {
            7: 5, 8: 7, 9: 4,  # Matin
            16: 4, 17: 5, 18: 7, 19: 4,  # Soir
            12: 3, 13: 3  # Midi
        }
